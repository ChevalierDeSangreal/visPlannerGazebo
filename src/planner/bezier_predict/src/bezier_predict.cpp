...
MatrixXd Bezierpredict::getQ(const int vars_number, const vector<double> Time, const int seg_index){
...
int Bezierpredict::TrackingGeneration(
    ...
    for(int i = 0; i < _MAX_SEG; i++){
        ...
        if(!tanh_input){
            ...
            history_weight_list.push_back(tanh(0.6 * tanh_input));
    ...
    //等式约束部分
    int my = 3;                             // 线性等式约束的个数(end_p)
    double b[3];                            // 线性等式约束的右侧向量b
    b[0] = end_p[0];   //end_p
    b[1] = end_p[1];
    b[2] = end_p[2];
    int nnzA = vars_number * 3;             // nonze-ros总数
    int irowA[nnzA];                         // 等式约束中的A
    int jcolA[nnzA];
    double dA[nnzA];
    int nn_idx = 0;
    int row_idx = 0;
    ...
    int nnzC =segs * (vars_number-1)*2 *3 + segs * (vars_number-2)*3 *3;//速度加速度约束

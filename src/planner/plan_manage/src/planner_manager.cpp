...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    if( pp_.drone_id == 0 )
    {
      bezier_predictor_.reset(new Predictor);
      bezier_predictor_->setParam(nh, visualization_);
      bezier_predictor_->setEnvironment(grid_map_);
  ...
  void EGOPlannerManager::angleLimite( double &angle )
  {
    while( angle > M_PI )
    {
      angle -= (M_PI * 2);
  ...
  bool EGOPlannerManager::reboundReplanWithYaw(Eigen::Vector3d start_yaw, Eigen::Vector3d target_vel)
  {
    ROS_INFO("plan with yaw");
    auto t1 = ros::Time::now();
    
    auto&  pos = local_data_.position_traj_;
    double dt_yaw = pos.getInterval();
    double init_yaw = start_yaw(0);
    start_yaw(0) = 0;

    angleLimite(init_yaw);

    // calculate initial control points with boundary state constraints
    int pts_num = pos.getControlPoint().cols();

    Eigen::MatrixXd yaw_control_points(3, pts_num);
    yaw_control_points.setZero();

    Eigen::Matrix3d states2pts;
    states2pts << 1.0, -dt_yaw, (1 / 3.0) * dt_yaw * dt_yaw, 1.0, 0.0, -(1 / 6.0) * dt_yaw * dt_yaw, 1.0,
        dt_yaw, (1 / 3.0) * dt_yaw * dt_yaw;
    yaw_control_points.block(0, 0, 1, 3) = (states2pts * start_yaw).transpose();
    
    // solve
    Eigen::MatrixXd ctrl_pts = pos.getControlPoint();
    bspline_optimizer_->initControlPoints_yaw(yaw_control_points);

    double target_vel_yaw = atan2(target_vel(1), target_vel(0));
    target_vel_yaw = target_vel_yaw - init_yaw;

    angleLimite(target_vel_yaw);

    bspline_optimizer_->setVelYaw(target_vel_yaw);
    // bspline_optimizer_->setWaypoints_yaw(waypts, waypt_idx);
    Eigen::MatrixXd pos_control_points;
    bool yaw_plan_success = bspline_optimizer_->BsplineOptimizeTrajReboundYaw(pos_control_points, yaw_control_points, dt_yaw, init_yaw);
    // bool yaw_plan_success = bspline_optimizer_->BsplineOptimizeTrajReboundVisibility(pos_control_points, dt_yaw);
    for( int i = 0; i < pts_num; i++ )
    {
      yaw_control_points(0,i) = yaw_control_points(0,i) + init_yaw;
    }

    //update
    auto pos_traj = UniformBspline(pos_control_points, 3, dt_yaw);
    auto yaw_traj = UniformBspline(yaw_control_points, 3, dt_yaw);
    updateTrajInfo(pos_traj, ros::Time::now());
    updateYawTrajInfo(yaw_traj, ros::Time::now());
    // updateIndexInFreespace( bspline_optimizer_->getIndexInFreespace() );
  ...
  void EGOPlannerManager::get_track_line( double t_cur, double t_max, double dt, double start_time_ros,
                                                     std::vector<Eigen::Vector3d> &attract_lines )
  {
    dt = dt;

    double max_dis = pp_.attract_max_dist_threshold_;
    double min_dis = pp_.attract_min_dist_threshold_;
    double resolution_inv = grid_map_->getResolution_inv();

    for( double t_now = t_cur; t_now < t_max-0.1; t_now = t_now + dt )
    {
      Eigen::Vector3d pos_now = local_data_.position_traj_.evaluateDeBoorT(t_now);
      double yaw_now = local_data_.yaw_traj_.evaluateDeBoorT(t_now)[0];

      double glb_time = start_time_ros + t_now;
      for (size_t id = 0; id < swarm_trajs_buf_.size(); id++)
      {
        if ( (swarm_trajs_buf_.at(id).drone_id != (int)id) || swarm_trajs_buf_.at(id).drone_id == pp_.drone_id )
        {
          continue;
        }
        double traj_i_satrt_time = swarm_trajs_buf_.at(id).start_time_.toSec();
        if (glb_time < traj_i_satrt_time + swarm_trajs_buf_.at(id).duration_ - 0.1)
        {
          Eigen::Vector3d pos_swarm = swarm_trajs_buf_.at(id).position_traj_.evaluateDeBoorT(glb_time - traj_i_satrt_time);

          attract_lines.push_back(pos_now);
          attract_lines.push_back(pos_swarm);
        }
      }
    }
  }
  ...
  bool EGOPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  bool EGOPlannerManager::reboundReplanForPredict(Eigen::MatrixXd &ctrl_pts, double ts)
  {
    ...
    bool plan_success = bspline_optimizer_->BsplineOptimizeTrajRebound(ctrl_pts, ts);    
  ...
  void EGOPlannerManager::updateYawTrajInfo(const UniformBspline &yaw_traj, const ros::Time time_now)
  {
    local_data_.start_time_ = time_now;
    local_data_.yaw_traj_ = yaw_traj;
    local_data_.yawdot_traj_ = local_data_.yaw_traj_.getDerivative();
    // local_data_.start_yaw_ = local_data_.yaw_traj.evaluateDeBoorT(0.0);
  }
